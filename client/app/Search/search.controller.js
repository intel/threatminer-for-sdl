...
angular.module('threat').controller('SearchController', function($http, $routeParams, $location, values) {
  ...
  var classTypeParam = $routeParams.classificationType;
  var classIdParam = $routeParams.classificationId;
  ...
  var getAdversaries = $http.get(values.get('api') + '/adversaries').then(function(response){
  ...
  var getAssets = $http.get(values.get('api') + '/assets').then(function(response){
  ...
  var getAttackTypes = $http.get(values.get('api') + '/attack_types').then(function(response){
  ...
  var getAttackVectors = $http.get(values.get('api') + '/attack_vectors').then(function(response){
  ...
  var getVulnerabilities = $http.get(values.get('api') + '/vulnerabilities').then(function(response){
    currentSortProp = "vuln_name";
    vm.vulnerabilities = response.data.sort(vm.sortAlphabetically);
  });

  // If a url
  Promise.all([getAdversaries, getAdversaries, getAttackTypes, getAttackVectors, getVulnerabilities]).then(function() {
    if (classTypeParam == "ADVERSARY") {
      var foundAdversary = vm.findClassification(vm.adversaries, "adv_id", parseInt(classIdParam));
      ...
      var foundAsset = vm.findClassification(vm.assets, "asset_id", parseInt(classIdParam));
      ...
      var foundAttackType = vm.findClassification(vm.attackTypes, "atktyp_id", parseInt(classIdParam));
      ...
      var foundAttackVector = vm.findClassification(vm.attackVectors, "atkvtr_id", parseInt(classIdParam));
      ...
      var foundVulnerability = vm.findClassification(vm.vulnerabilities, "vuln_id", parseInt(classIdParam));
  ...
  vm.addClassificationToSelected = function(classification, classificationName, classificationType, classificationTypePlural, classificationShortcode) {
    if (vm["selected" + vm.capitalizeFirstChar(classificationTypePlural)].filter(function(selectedClassification){ return selectedClassification[classificationShortcode + "_id"] === classification[classificationShortcode + "_id"]}).length == 0) {
  ...
  vm.updateSelector = function(classificationName, classificationType, classificationTypePlural, isSelecting) {
    var selections = angular.element("[nya-bs-option~='" + classificationType + "']");
  ...
  vm.removeSelection = function(shortCode, classificationName, classificationType, classificationTypePlural, isSelecting) {
    var selectedClassifcation = vm["selected" + vm.capitalizeFirstChar(classificationTypePlural)];
    for (var x = 0; x < selectedClassifcation.length; x++) {
  ...
  */
  vm.getAdvSearchDataAsArray = function() {
    return vm.getAllAdvancedSearchResultsPromise().then(function(advancedSearchResults) {
      var outputArr = [];
      var hasAdversaries = false;
      var hasAssets = false;
      var hasAttackTypes = false;
      var hasAttackVectors = false;
      var hasVulnerabilities = false;
      ...
      for (var x = 0; x < advancedSearchResults.length; x++) {
        var adversaries = vm.escapeCommas(vm.objArrToIndexedArrByKey(advancedSearchResults[x].adversaries, "adv_name")).join(', ');
        var assets = vm.escapeCommas(vm.objArrToIndexedArrByKey(advancedSearchResults[x].assets, "asset_name")).join(', ');
        var attackTypes = vm.escapeCommas(vm.objArrToIndexedArrByKey(advancedSearchResults[x].attack_types, "atktyp_name")).join(', ');
        var attackVectors = vm.escapeCommas(vm.objArrToIndexedArrByKey(advancedSearchResults[x].attack_vectors, "atkvtr_name")).join(', ');
        var vulnerabilities = vm.escapeCommas(vm.objArrToIndexedArrByKey(advancedSearchResults[x].vulnerabilities, "vuln_name")).join(', ');
        var threatTitle = advancedSearchResults[x].threat[0]["threat_title"];
        var threatLink = "http://" + window.location.host + "/threats/" + advancedSearchResults[x].threat[0]["threat_id"];
        ...
        outputArr[x] = {"Adversaries":adversaries, "Assets":assets, "Attack Types":attackTypes, "Attack Vectors":attackVectors, "Vulnerabilities":vulnerabilities, "Threat Title":threatTitle, "Threat Link":threatLink};
      }
  ...
  vm.getSearchDataAsArray = function() {
    var apiString = values.get('api') + '/threats/type/';
    if (vm.searchQuery != "") {
    ...
    return $http.post(apiString, ({searchQuery:vm.searchQuery, sortDirection:vm.curBasicSortDirection, sortField:vm.curBasicSortKey})).then(function(response){
      var data = response.data;
      var threat_date = null;
      var threat_title = null;
      var threat_desc = null;
      var threat_link = null;
      data.forEach(function(element, index) {
        threat_date = vm.formatDateString(element.threat_date);
        threat_title = element.threat_title;
        threat_desc = element.threat_desc;
        threat_link = "http://" + window.location.host + "/" + element.threat_id;
        data[index] = {"threat_date":threat_date, "threat_title":threat_title, "threat_desc":threat_desc, "threat_link":threat_link};
      });
		  return data;
	  });
  }
  ...
  vm.getAllAdvancedSearchResultsPromise = function() {
    ...
    var adversaries = vm.objArrToIndexedArrByKey(vm.selectedAdversariesBuffered , "adv_id");
    var assets = vm.objArrToIndexedArrByKey(vm.selectedAssetsBuffered , "asset_id");
    var attackTypes = vm.objArrToIndexedArrByKey(vm.selectedAttackTypesBuffered , "atktyp_id");
    var attackVectors = vm.objArrToIndexedArrByKey(vm.selectedAttackVectorsBuffered , "atkvtr_id");
    var vulnerabilities = vm.objArrToIndexedArrByKey(vm.selectedVulnerabilitiesBuffered , "vuln_id");
    classifications = {"adversaries":adversaries, "assets":assets, "attackTypes":attackTypes, "attackVectors":attackVectors, "vulnerabilities":vulnerabilities};
    inclusivity = {"adversaries":vm.adversariesInclusive, "assets":vm.assetsInclusive, "attackTypes":vm.attackTypesInclusive, "attackVectors":vm.attackVectorsInclusive, "vulnerabilities":vm.vulnerabilitiesInclusive};
    return $http.post(values.get('api') + '/threats/advanced', ({classifications:classifications, inclusivity:inclusivity, isCounting:false, pageNumber:null})).then(function(response){
				vm.exportLoading = false;
  ...
  vm.advancedSearch = function(currentPage) {
    ...
    var vulnerabilities = vm.objArrToIndexedArrByKey(vm.selectedVulnerabilitiesBuffered , "vuln_id");
    classifications = {"adversaries":adversaries, "assets":assets, "attackTypes":attackTypes, "attackVectors":attackVectors, "vulnerabilities":vulnerabilities};
    inclusivity = {"adversaries":vm.adversariesInclusiveBuffered, "assets":vm.assetsInclusiveBuffered, "attackTypes":vm.attackTypesInclusiveBuffered, "attackVectors":vm.attackVectorsInclusiveBuffered, "vulnerabilities":vm.vulnerabilitiesInclusiveBuffered};
		$http.post(values.get('api') + '/threats/advanced', ({classifications:classifications, inclusivity:inclusivity, isCounting:true, pageNumber:null})).then(function(response){
				vm.maxPageCountAdvanced = response.data;
				console.log(vm.maxPageCountAdvanced);
				$http.post(values.get('api') + '/threats/advanced', ({classifications:classifications, inclusivity:inclusivity, isCounting:false, pageNumber:vm.currentPageAdvanced})).then(function(response){
  ...
  vm.boldQueryInResults = function(textToReplaceIn, currentSearchParam) {
    if (textToReplaceIn) {
      var matchedString = new RegExp(vm.searchedQuery, 'gi').exec(textToReplaceIn);
      // If there is a matched string and checks that currentSearchParam matches
      var regex = new RegExp(vm.searchedQuery, 'gi');
  ...
  vm.formatDateString = function(dateString) {
    ...
    if (!currentPage) {
      ...
      vm.currentPage = 1;
		...
		var apiString = values.get('api') + '/threats/currentPage/' + currentPage + '/type/';
    var pageCountAPIString = values.get('api') + '/threats/pageCount/';
		if (vm.searchQuery != "") {
    ...
    $http.post(pageCountAPIString, ({searchQuery:vm.searchedQuery})).then(function(response){
        vm.maxPageCount = response.data;
    });
    // clear out the table and insert result of search into table
		$http.post(apiString, ({searchQuery:vm.searchedQuery, sortDirection:vm.curBasicSortDirection, sortField:vm.curBasicSortKey})).then(function(response){
			var dataArr = response.data;
			vm.displayedCollection = [];
			dataArr.forEach(function(element) {
				vm.displayedCollection.unshift({ID:element.threat_id, Date:vm.formatDateString(element.threat_date), Title:element.threat_title, Description:element.threat_desc})
			})
