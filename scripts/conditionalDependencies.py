...
#returns a stringof the most important words in a
#piece of text
...
#if fails, error is printed to .txt file
def run():
	try:
		print("Started")
		DBconnect()
		...
		file = open("ConditionalDependencyErrors.txt", "w")
...
def initProductsandFeeds():
	...
	cursor.execute("select * from products p , productCategories c  where p.category_id = c.category_id and (c.category_name='3rd Party COTS - Conditional' or c.category_name='Open Source - Conditional')")
	global products
...
def insert(product, title, summary, source, feed_id):
	...
	if rowCount == 0:
		#if classifier determines as relevant
		if(relevance == 'relevant'):
			...
			try:
				...
				pass
		#if classifier determines as not relevant
...
#takes 2 parameters and finds a whole word in another string
def findWholeWord(w):
	...
	for product in products:
		...
		for link in feeds:
			...
			for j in range(0,len(currLink['entries'])):
				...
				#proceed only if there is a title for the entry
				if(title != ""):
					#look for product name in title and desc
					nameFound= None
					#get all of the impoortant words using the RAKE method
					...
					for word in keywords:
						...
						try:
							...
							pass
						#if the keyoword is found in the important words and the source isnt empty, insert it into the database
						if(wordFound != None):
							hits+=1
							if(source != ""):
								insert(product, title, desc, source, link['feed_id'])
